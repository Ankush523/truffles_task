...
import UserInfo from "@/components/LoginPageComponents/UserInfo";
import BusinessInfo from "@/components/LoginPageComponents/BusinessInfo";
...
const LoginSidebar = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const status =
        typeof window !== "undefined" ? localStorage.getItem("select") : null;
      setStatus(status);
      console.log(status);
    }, 100);
  });
